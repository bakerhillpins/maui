...
namespace Microsoft.Maui.Controls
{
	...
	public abstract class BindableObject : INotifyPropertyChanged, IDynamicResourceHandler
	{
		...
		void SetValueActual(BindableProperty property, BindablePropertyContext context, object value, bool currentlyApplying, SetValueFlags attributes, SetterSpecificity specificity, bool silent = false)
		{
			...
			var bindingPair = context.Bindings.LastOrDefault();
			BindingBase binding = bindingPair.Value;
			
			if (binding != null)
			{
				BindingMode realizedMode = binding.GetRealizedMode(property);
				if (clearOneWayBindings && realizedMode is BindingMode.OneTime or BindingMode.OneWay ||
				    clearTwoWayBindings && realizedMode == BindingMode.TwoWay)
				{
					RemoveBinding(property, context, bindingPair.Key);
					binding = null;
				}
			}
